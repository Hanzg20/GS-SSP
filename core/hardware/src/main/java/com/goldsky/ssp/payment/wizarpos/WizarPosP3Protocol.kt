...
object WizarPosP3Protocol {
    ...
    /**
     * Packs a payload (String or ByteArray) into a P3 frame.
     */
    fun pack(path: Byte, sequence: Int, payload: Any): ByteArray {
        val contentBytes = when (payload) {
            is String -> payload.toByteArray(Charsets.UTF_8)
            is ByteArray -> payload
            else -> ByteArray(0)
        }
        
        // Size: STX(1) + VER(1) + CTRL(4) + LEN(2) + CONTENT(N) + ETX(1) + BCC(1) = N + 10
        val frameSize = contentBytes.size + 10
    ...
    /**
     * Unpacks a P3 frame and extracts the CONTENT.
     */
    fun unpackBytes(frame: ByteArray): ByteArray? {
        ...
        if (frame.size != contentLen + 10) {
            Log.e(TAG, "Frame size mismatch. Expected: ${contentLen + 10}, Actual: ${frame.size}")
            return null
        }

        if (frame[contentLen + 8] != ETX) {
            Log.e(TAG, "Invalid ETX: ${frame[contentLen + 8]}")
            return null
        }
        
        val receivedBcc = frame[receivedBccIdx(frame)]
        ...
        System.arraycopy(frame, 8, content, 0, contentLen)
        return content
    }

    private fun receivedBccIdx(frame: ByteArray): Int = frame.size - 1

    /**
     * Unpacks a P3 frame and extracts the JSON payload as String.
     */
    fun unpack(frame: ByteArray): String? {
        val bytes = unpackBytes(frame) ?: return null
