package com.goldsky.ssp.iris.hardware.idtech
...
class IdTechHardwareProvider : IHardwareProvider, DefaultLifecycleObserver {
    ...
    override fun getGpioProvider(): IGpioProvider = object : IGpioProvider {
        // ID TECH is a card-reader vendor with no relay/GPIO hardware of its
        // own -- false (not true) so a caller that actually depends on a
        // pulse (DigitIoAdapter) can't mistake this unsupported stub for a
        // real pulse having happened.
        override fun setRelay(port: Int, on: Boolean): Boolean = false
    ...
    override fun init(context: Context) {
        if (reader == null) {
            try {
                ...
                // Throwable, not Exception: see PaxHardwareProvider's init()
                // for why -- every app shell registers all 3 vendor providers
                // unconditionally, so this must degrade gracefully on
                // non-ID-TECH hardware too instead of taking down the whole app.
    ...
    override fun reboot(): Boolean {
        Log.w(TAG, "REBOOT unsupported: ID TECH peripheral cannot reboot the host terminal")
        return false
    }
