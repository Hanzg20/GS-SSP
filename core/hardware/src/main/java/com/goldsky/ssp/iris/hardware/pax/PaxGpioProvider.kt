...
class PaxGpioProvider : IGpioProvider {
    ...
    override fun setRelay(port: Int, on: Boolean): Boolean {
        return try {
            ...
            // Throwable, not Exception: same missing-native-lib class of
            // fault as PaxHardwareProvider.init() -- DigitalIOManager's own
            // static init can throw UnsatisfiedLinkError, an Error not an
            // Exception, on hardware without PAX's native libs.
