...
| **4.2 伪造/猜测券码** | 券码（`code` 列）生成时必须用**不可预测的随机字符串**。**2026-09-19 更新**：为方便打印/屏幕显示/人工朗读，券码长度收紧为**不超过 10 位**（目前实际生成 8 位，从 32 字符集随机取，去除易混淆的 0/O/1/I/L，约 40 bits 随机性），不再要求 16+ 位；仍然不用 `COUPON0001` 这种连续编号。40 bits 熵在"单次核销、按 org 隔离、且核销走设备扫码而非公网批量试错接口"的威胁模型下足够，但比旧的 UUID 方案余量更小，如果未来核销量级大幅上升或暴露出公网可批量调用的核销入口，需要重新评估位数。核销时"这个码根本不存在"和"存在但已用完"分别返回 `not_found`/`already_used`，两者都归为面向客户的同一句"此券无法使用"（见 4.6），避免暴露信息帮助攻击者判断哪些码"曾经有效"。 |
