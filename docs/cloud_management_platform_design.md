...
*   **Member QR Enrollment**:
    *   Automated generation of 6-character alphanumeric codes (text, not numeric-only -- revised 2026-07-24; shortened from 12 chars on 2026-09-20) for every `vip_cards` record. **Confirmed distinct from `card_uid`** (existing seed data like `"VIP_CARD_UID_6789"` doesn't match this format either) -- this needs its own new column, e.g. `vip_cards.qr_code TEXT UNIQUE`, resolved to `card_uid` by the terminal before calling `deduct_vip_balance()`. See `docs/coupon_redemption_integration.md` §2.1 for the terminal-side lookup this implies.
...
*   **Asset Management**: Centralized repository of videos, images, and full-screen text announcements. `[UPDATED 2026-07-28]` -- `advertisements.media_type` now also accepts `'TEXT'`, with `media_url` relaxed to nullable and a new `text_content` column (a `CHECK` constraint enforces `media_url` required for VIDEO/IMAGE, `text_content` required for TEXT). The CMP automatically calculates **MD5 Fingerprints** for every video/image file to enable incremental terminal-side syncing and integrity verification -- TEXT rows have no file, so `md5_hash` stays null and `AdSyncWorker.kt` skips the download/verify step for them entirely, syncing just the row's text via the same playlist-cache JSON. `AdActivity.kt` renders TEXT rows as a visually distinct glass-card "announcement" panel (royal-blue label, light body text) rather than reusing the bold amber video/image ad styling, and both ad and announcement playback now have a small pause/resume control (see below).
*   **Playlist Orchestration**: 
    ...
    *   `play_order` allows for precise sequencing of marketing loops.
    ...
    `[INACCURATE -- NOT ACTUALLY WIRED UP, 2026-07-28]` The `playlists` table (`device_sn`, `ad_id`, `play_order`) exists in `docs/supabase_full_schema.sql`, but `AdSyncWorker.kt`'s sync query (`SupabaseClientProvider.client.postgrest["advertisements"].select()`) pulls **every** row in `advertisements` unconditionally -- it never joins through `playlists`. Every device today receives every ad/announcement globally, in whatever order Postgres happens to return them; there is no vertical- or site-level targeting, and `play_order` is not read by the client at all. "Operators group assets into playlists" describes a workflow with no supporting UI (no CMP portal exists, per the reality check at the top of this doc) and no client-side enforcement even if it did. Building this for real means changing the sync query to `SELECT ... FROM advertisements JOIN playlists ON ... WHERE playlists.device_sn = <this device>` (deciding what "untargeted" should mean -- global fallback, or excluded -- is a product call, not just a query change).
*   **Delta Sync Control**: The system only notifies terminals to download a file if the cloud MD5 differs from the last-reported local MD5, significantly reducing data usage. `[MATCHES CURRENT SCHEMA]` -- accurately describes `AdSyncWorker.md5Matches()` for VIDEO/IMAGE rows.
*   **Viewer Pause Control** `[NEW, BUILT 2026-07-28, not in original design]` -- a small corner pause/resume button on `AdActivity` lets a customer freeze the current video or announcement instead of being forced through the timed rotation. Deliberately a small non-blocking control, not a full-screen gesture: the rest of the screen still has to work as the "tap to start a wash" CTA, since the terminal's business goal is conversions, not ad dwell-time. Purely client-side UX; no CMP/backend involvement.

#### 3.5.2 Playback Monitoring & Reporting
*   **Proof of Play**: CMP aggregates `play_event` signals from terminals to provide reach and exposure reports to advertisers. `[NOT BUILT]` -- no `play_event` table or equivalent exists anywhere in the schema, and neither `AdActivity.kt` nor `AdSyncWorker.kt` emits any playback telemetry. This is pure vision with zero implementation, same bucket as §3.6/§3.7.
*   **Operational Health**: Visual indicators for machines that have failed to download their assigned assets (e.g., due to "Storage Full" or "Low Signal"). `[NOT BUILT]` -- ad download failures are only `Log.e`'d locally in `AdSyncWorker`; nothing is written to `app_error_logs` or surfaced anywhere remotely. `DiagnosticManager.kt` already exists elsewhere in the app for exactly this kind of reporting, but `AdSyncWorker` doesn't call it -- wiring that up is the natural next step before a real "Operational Health" view is possible.
